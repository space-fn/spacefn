...
function generatePagesCode(pages: ScannedPage[], root: string): string {
	...
	for (let i = 0; i < pages.length; i++) {
		const p = pages[i];
		if (p.serverFile) {
			const rel = relative(root, p.serverFile).replace(/\.ts$/, "");
			lines.push(`import * as Server_${i} from "#${rel}";`);
		}
		if (p.pageFile) {
			const rel = relative(root, p.pageFile).replace(/\.ts$/, "");
	...
	for (let i = 0; i < pages.length; i++) {
		const p = pages[i];
		const hasLoader = p.serverFile !== null && p.hasLoader;
		const hasActions = p.serverFile !== null && p.hasActions;
		const hasPage = p.pageFile !== null;

		lines.push(`\t{`);
		lines.push(`\t\tpattern: "${p.pattern}",`);
		lines.push(`\t\tloader: ${hasLoader ? `Server_${i}.loader` : "undefined"},`);
		lines.push(`\t\tactions: ${hasActions ? `Server_${i}.actions` : "undefined"},`);
		if (hasPage) {
...
function generateMiddlewaresCode(middlewares: ScannedMiddleware[]): string {
	...
	lines.push("];");
	lines.push("export default middlewares;");
	lines.push("");
...
export function space(options: SpacePluginOptions = {}): Plugin[] {
	...
	const pagesGenerator: Generator = {
		...
		async generate() {
			...
			return generatePagesCode(pages, root);
		},
	};
				...
				try {
						...
						for (const page of pagesMod.pages) {
							if (matchPattern(page.pattern, url.pathname)) {
								const data = page.loader
									? await page.loader(new Request(url.toString(), { method: req.method }))
									: undefined;
					...
					if (routesMod?.routes) {
						for (const route of routesMod.routes) {
							if (
								matchPattern(route.pattern, url.pathname) &&
